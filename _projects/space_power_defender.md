---
title: 'Space Power Defender'
description: "Space shooter where you should use your spaceship to defend your power orbs from enemy waves. Each orb is responsible for one power (speed, shield, side bullets, regeneration) on your spaceship."


header: '/assets/images/projects/spd/space_power_defender.png'
thumbnail: '/assets/images/projects/spd/01.gif'
images: ['/assets/images/projects/spd/01.gif',
        '/assets/images/projects/spd/02.gif',
        '/assets/images/projects/spd/03.gif',
        '/assets/images/projects/spd/04.gif']
link: 'https://gamejolt.com/games/space-power-defender/283201'
play_now: 'https://widgets.gamejolt.com/package/v1?key=kwnJArAq'

technologies: ['Unity', 'Pixel Art']
categories: ['game jam', 'games']

layout: project
nav_type: project_page_nav
---